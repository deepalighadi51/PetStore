...
def create_order_pdf(order,cart_items):
    ...
    for item in cart_items:
        item_data.append([item.pet.name,item.quantity, item.pet.price*item.quantity])     
...
def send_order_email(order, cart_items):
    ...
    email_body = "Thank-You For Placeing the Order, Please find the attached invoice for your reference."
    pdf_content = create_order_pdf(order, cart_items)

    #attachment of pdf
    pdf_attachement = ("Order_invoice.pdf", pdf_content, "application/pdf")
